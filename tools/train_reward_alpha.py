...
from roi_data.loader_SS import RoiDataLoader, MinibatchSampler, BatchSampler, collate_minibatch
from modeling.model_reward_alpha_builder import Generalized_RCNN
...
def main():
    ...
    maskRCNN = mynn.DataParallel(maskRCNN, cpu_keywords=['im_info', 'roi'],
                                 minibatch=True)
        ...
        for step in range(args.start_step, cfg.SOLVER.MAX_ITER):
            ...
            for inner_iter in range(args.iter_size):
                ...
                for key in input_data:
                    if key != 'roidb' and key != 'ori_data' and key != 'im_info' and key != 'superpixels': # roidb is a list of ndarrays with inconsistent length
                        input_data[key] = list(map(Variable, input_data[key]))

                # add alpha
                #input_data['alpha'] = [1.0 - (step + 1.0) / cfg.SOLVER.MAX_ITER] * 4
                #input_data['alpha'] = [0.5 * (1 + np.cos(step * np.pi / cfg.SOLVER.MAX_ITER))] * 4 
                tmp = 1. / cfg.SOLVER.MAX_ITER * np.log(1.0 / 1e-2)
                input_data['alpha'] = [np.exp(-tmp * step)] * 4
                #
