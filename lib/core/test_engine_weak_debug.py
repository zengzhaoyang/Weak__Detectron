...
from core.test_weak_debug import im_detect_all
from datasets import task_evaluation
from datasets.json_dataset import JsonDataset
from modeling import model_weak_debug_builder
import nn as mynn
...
def test_net(
    ...
    for i, entry in enumerate(roidb):
        ...
        boxes, features = im_detect_all(model, im, box_proposals, timers)

        pickle.dump((boxes, features), open('debug/%d.pkl'%i, 'wb'))    
...
def initialize_model_from_cfg(args, gpu_id=0):
    ...
    """
    model = model_weak_debug_builder.Generalized_RCNN()
    model.eval()
