...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        ...
        features, scores, boxes, boxes_predict = im_detect_bbox(
    ...
    return scores[:, 1:], boxes, features, boxes_predict
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None, return_index=False):
    ...
    features = return_dict['conv5_feature'].data.cpu().numpy()
    ...
    if return_index:
        ...
        return features, scores, boxes[inv_index, :], box_deltas[inv_index, :]
