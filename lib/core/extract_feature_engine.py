...
from core.extract_feature import im_detect_all
...
def test_net(
    ...
    for i, entry in enumerate(roidb):
        ...
        scores, boxes, features, bbox_predict = im_detect_all(model, im, box_proposals, timers)

        fe = features.squeeze().mean(axis=0)
        det = fe.max() - fe.min()
        ratio = 255. / det
        fe = (fe - fe.min()) * ratio
        fe = fe.astype(np.uint8) 
        fe = fe[:, :, None]
        fe = np.concatenate([fe, fe, fe], axis=2)
        #np.save('features/%d_scores.npy'%i, scores)
        #np.save('features/%d_boxes.npy'%i, boxes)
        np.save('features/%d_features.npy'%i, fe)
        #np.save('features/%d_bbox_predict.npy'%i, bbox_predict)
