...
class reward_outputs(nn.Module):
    def __init__(self, dim_in):
        ...
        self.reward_pred = nn.Linear(dim_in, 1)
    ...
    def _init_weights(self):
        ...
        init.normal_(self.reward_pred.weight, std=0.001)
        init.constant_(self.reward_pred.bias, 0)
    ...
    def detectron_weight_mapping(self):
        detectron_weight_mapping = {
            ...
            'reward_pred.weight': 'reward_pred_w',
            'reward_pred.bias': 'reward_pred_b'
        }
        orphan_in_detectron = []
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            return bbox_mul, cls_refine1, cls_refine2, cls_refine3, bbox_pred, reward_pred
        else:
...
def reward_losses(rois, images, superpixels, im_info, bbox_mul, label_int32, cls_refine1, cls_refine2, cls_refine3, bbox_pred, reward_pred, alpha):
    ...
    def _sample_rois(all_rois, proposals, with_bbox=False):
        ...
        if with_bbox:
            ...
            bbox_inside_weights[fg_inds, 4:8] = (cls_loss_weights[fg_inds] > 0).astype(np.float32)
    ...
    label1, _ori_label = _sample_rois(rois_npy, proposals1)

    pos_idx = np.where(_ori_label != 0)[0]

    #reward1 = cal_reward(images, superpixels, im_info, rois_npy[pos_idx, :], 'ED', 2)
    #reward2 = cal_reward(images, superpixels, im_info, rois_npy[pos_idx, :], 'CC', 2)
    #reward3 = cal_reward(images, superpixels, im_info, rois_npy[pos_idx, :], 'MS', 0.2)
    reward = cal_reward(images, superpixels,  im_info, rois_npy[pos_idx, :], 'SS')
    reward = Variable(torch.from_numpy(reward)).cuda().float() # r * 21

    reward_pred = reward_pred[pos_idx, :]
    reward_weight = reward_pred.detach()
    reward_weight = torch.clamp(reward_weight, 0, 2)

    reward_weight = reward * alpha + reward_weight * (1 - alpha)

    label1 = Variable(torch.from_numpy(label1)).cuda().float() # r * 21
    label1[pos_idx, :] *= reward_weight
    ...
    label2, _ = _sample_rois(rois_npy, proposals2)
    label2 = Variable(torch.from_numpy(label2)).cuda().float() # r * 21
    label2[pos_idx, :] *= reward
    ...
    bbox_inside_weights[pos_idx, :] *= reward_weight
    ...
    reward_diff = torch.pow(reward_pred - reward, 2)
    loss_reward = 10. * reward_diff.view(-1).sum(0) / rois_npy.shape[0]

    return cls_loss, refine_loss1, refine_loss2, refine_loss3, loss_bbox, loss_reward
