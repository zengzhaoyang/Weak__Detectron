...
def VGG16_conv5_trident_body():
    return VGG_conv5_trident_body()
...
class VGG_conv5_trident_body(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1_1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv1_2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True)
        ...
        self.conv5_1_d2 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=2, dilation=2, bias=True)
        self.conv5_1_d2.weight = self.conv5_1.weight
        self.conv5_1_d2.bias = self.conv5_1.bias
        self.conv5_2_d2 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=2, dilation=2, bias=True)
        self.conv5_2_d2.weight = self.conv5_2.weight
        self.conv5_2_d2.bias = self.conv5_2.bias
        self.conv5_3_d2 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=2, dilation=2, bias=True)
        self.conv5_3_d2.weight = self.conv5_3.weight
        self.conv5_3_d2.bias = self.conv5_3.bias

        self.conv5_1_d3 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=3, dilation=3, bias=True)
        self.conv5_1_d3.weight = self.conv5_1.weight
        self.conv5_1_d3.bias = self.conv5_1.bias
        self.conv5_2_d3 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=3, dilation=3, bias=True)
        self.conv5_2_d3.weight = self.conv5_2.weight
        self.conv5_2_d3.bias = self.conv5_2.bias
        self.conv5_3_d3 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=3, dilation=3, bias=True)
        self.conv5_3_d3.weight = self.conv5_3.weight
        self.conv5_3_d3.bias = self.conv5_3.bias
    ...
    def train(self, mode=True):
        ...
        self.conv5_1_d2.train(mode)
        self.conv5_2_d2.train(mode)
        self.conv5_3_d2.train(mode)
        self.conv5_1_d3.train(mode)
        self.conv5_2_d3.train(mode)
        self.conv5_3_d3.train(mode)
    ...
    def forward(self, x):
        ...
        x_d1 = F.relu(self.conv5_1(x), inplace=True)
        x_d1 = F.relu(self.conv5_2(x_d1), inplace=True)
        x_d1 = F.relu(self.conv5_3(x_d1), inplace=True)

        x_d2 = F.relu(self.conv5_1_d2(x), inplace=True)
        x_d2 = F.relu(self.conv5_2_d2(x_d2), inplace=True)
        x_d2 = F.relu(self.conv5_3_d2(x_d2), inplace=True)

        x_d3 = F.relu(self.conv5_1_d3(x), inplace=True)
        x_d3 = F.relu(self.conv5_2_d3(x_d3), inplace=True)
        x_d3 = F.relu(self.conv5_3_d3(x_d3), inplace=True)
...
class VGG_roi_fc_trident_head(nn.Module):
    """Add a ReLU MLP with two hidden layers."""
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        super().__init__()
        self.dim_in = dim_in
...
class VGG_roi_fc_cat_head(nn.Module):
    """Add a ReLU MLP with two hidden layers."""
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        super().__init__()
        self.dim_in = dim_in
        ...
        self.fc1 = nn.Linear(dim_in * self.roi_size**2, hidden_dim)
        self.dropout1 = nn.Dropout(0.5, inplace=True)
        self.fc2 = nn.Linear(hidden_dim * 5, hidden_dim)
        self.dropout2 = nn.Dropout(0.5, inplace=True)
    ...
    def forward(self, x, rpn_ret):
        ...
        xs = [F.relu(self.fc1(x.view(batch_size, -1)), inplace=True) for x in xs]
        x = torch.cat(xs, 1)
