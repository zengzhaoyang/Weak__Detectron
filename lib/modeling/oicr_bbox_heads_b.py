...
def oicr_losses(rois, bbox_mul, label_int32, cls_refine1, cls_refine2, cls_refine3, bbox_pred):
    ...
    refine_loss1 = oicr1(rois_npy, bbox_mul.detach().cpu().numpy(), img_label, cls_refine1, False)
    refine_loss2 = oicr2(rois_npy, cls_refine1_softmax[:, 1:].detach().cpu().numpy(), img_label, cls_refine2, False)
    refine_loss3 = oicr3(rois_npy, cls_refine2_softmax[:, 1:].detach().cpu().numpy(), img_label, cls_refine3, True, bboxs)
