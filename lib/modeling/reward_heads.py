...
def cal_reward(image, im_info, roi, reward_type, theta):
...
def reward_losses(rois, images, im_info, bbox_mul, label_int32, cls_refine1, cls_refine2, cls_refine3, bbox_pred):
    ...
    reward1 = cal_reward(images, im_info, rois_npy[pos_idx, :], 'ED', 2)
    #reward2 = cal_reward(images, im_info, rois_npy[pos_idx, :], 'CC', 2)
    reward3 = cal_reward(images, im_info, rois_npy[pos_idx, :], 'MS', 0.2)

    reward = reward1 + reward3
    #reward = cal_reward(images, im_info, rois_npy[pos_idx, :], 'MS', 0.2)
    #print(reward)
    reward = Variable(torch.from_numpy(reward)).cuda().float() # r * 21

    label1 = Variable(torch.from_numpy(label1)).cuda().float() # r * 21
    label1[pos_idx, :] *= reward
    ...
    bbox_inside_weights[pos_idx, :] *= reward
