...
import modeling.reward_heads as reward_heads
...
class Generalized_RCNN(nn.Module):
    ...
    def forward(self, data, ori_data, im_info=None, rois=None, labels_int32=None):
        ori_data = ori_data.cpu().numpy()[0]
        im_info = im_info[0].cpu().numpy()
        if cfg.PYTORCH_VERSION_LESS_THAN_040:
            return self._forward(data, ori_data, im_info, rois, labels_int32)
        else:
            with torch.set_grad_enabled(self.training):
                return self._forward(data, ori_data, im_info, rois, labels_int32)

    def _forward(self, data, ori_data, im_info, rois, labels_int32):
        im_data = data
        ...
        if self.training:
            ...
            cls_loss, refine_loss1, refine_loss2, refine_loss3, bbox_loss = reward_heads.reward_losses(rois, ori_data, im_info, bbox_mul, labels_int32, cls_refine1, cls_refine2, cls_refine3, bbox_pred)
