...
import modeling.reward_learn_heads as reward_heads
...
class Generalized_RCNN(nn.Module):
    ...
    def forward(self, data, ori_data, superpixels, im_info=None, rois=None, labels_int32=None, alpha=1.0):

        if not ori_data is None:
            ori_data = ori_data.cpu().numpy()[0]
            superpixels = superpixels.cpu().numpy()

        im_info = im_info[0].cpu().numpy()
        if cfg.PYTORCH_VERSION_LESS_THAN_040:
            return self._forward(data, ori_data, superpixels, im_info, rois, labels_int32, alpha)
        else:
            with torch.set_grad_enabled(self.training):
                return self._forward(data, ori_data, superpixels, im_info, rois, labels_int32, alpha)

    def _forward(self, data, ori_data, superpixels, im_info, rois, labels_int32, alpha):
        im_data = data
        ...
        if self.training:
            ...
            bbox_mul, cls_refine1, cls_refine2, cls_refine3, bbox_pred, reward_pred = self.Box_Outs(box_feat)
            ...
            cls_loss, refine_loss1, refine_loss2, refine_loss3, bbox_loss, reward_loss = reward_heads.reward_losses(rois, ori_data, superpixels, im_info, bbox_mul, labels_int32, cls_refine1, cls_refine2, cls_refine3, bbox_pred, reward_pred, alpha)
            ...
            return_dict['losses']['reward_loss'] = reward_loss
