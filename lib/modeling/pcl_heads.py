...
def _get_proposal_clusters(all_rois, proposals, im_labels, cls_prob, with_bbox=False):
    num_classes = im_labels.shape[0]
    ...
    if with_bbox:
        ...
        bbox_targets = Variable(torch.from_numpy(bbox_targets), requires_grad=False).cuda().float()
        bbox_inside_weights = Variable(torch.from_numpy(bbox_inside_weights), requires_grad=False).cuda().float()
        bbox_outside_weights = Variable(torch.from_numpy(bbox_outside_weights), requires_grad=False).cuda().float()
    ...
    if with_bbox:
        return refine_loss / cls_prob.shape[0], labels, cls_loss_weights, gt_assigment, pc_labels, pc_probs, pc_count, img_cls_loss_weights, bbox_targets, bbox_inside_weights, bbox_outside_weights
    else:
...
class PCL(torch.autograd.Function):
    ...
    def forward(ctx, rois_npy, cls_prob, img_label, cls_refine, with_bbox=False, bboxs=[]):
        ...
        if with_bbox:
            loss, labels, cls_loss_weights, gt_assigment, pc_labels, pc_probs, pc_count, img_cls_loss_weights, bbox_targets, bbox_inside_weights, bbox_outside_weights = _get_proposal_clusters(rois_npy, proposals, img_label, cls_refine.detach().cpu().numpy(), with_bbox)
            bboxs.append(bbox_targets)
            bboxs.append(bbox_inside_weights)
            bboxs.append(bbox_outside_weights)
        else:
            loss, labels, cls_loss_weights, gt_assigment, pc_labels, pc_probs, pc_count, img_cls_loss_weights = _get_proposal_clusters(rois_npy, proposals, img_label, cls_refine.detach().cpu().numpy(), with_bbox)
