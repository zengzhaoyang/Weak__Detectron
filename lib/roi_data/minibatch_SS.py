...
def get_minibatch(roidb, preloads=None):
    ...
    im_blob, im_scales, ori_ims, superpixels = _get_image_blob(roidb, preloads)
    blobs['data'] = im_blob
    blobs['ori_data'] = ori_ims
    blobs['superpixels'] = superpixels
...
def _get_image_blob(roidb, preloads=None):
    ...
    for i in range(num_images):
        if preloads is None:
            ...
            sp = cv2.imread(roidb[i]['image'].replace('JPEGImages', 'superpixels'))
        #else:
    ...
    return blob, im_scales, ori_ims, superpixels
